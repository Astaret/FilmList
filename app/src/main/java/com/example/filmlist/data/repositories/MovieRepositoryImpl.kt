...
class MovieRepositoryImpl @Inject constructor(
    ...
    override suspend fun loadData(loadPage: Int): List<Movie> {
        return movieApiService.getTopRatedMovies(page = loadPage).MovieList.listMovieDtoToListMovie()
    }

    override suspend fun loadDataFromSearch(query: String): List<Movie> {
        return movieApiService.searchMovies(query).MovieList.map {
            it.dtoToMovie()
        }
    }

    override suspend fun getTotalPages(): Int {
        return movieApiService.getTopRatedMovies(page = 1).totalPages.toInt()
    }

    override suspend fun getMovieInfo(id: Int): Movie {
        return movieApiService.getMovieInfo(movieId = id).dtoToMovie()
    }
    ...
    override suspend fun getMovieListFromBd(state: ListMovieState): List<Movie> {
        return coroutineScope {
            movieDao.getMovieListFromBd(state).map { storeMovie ->
                async {
                    movieApiService.getMovieInfo(storeMovie.id).dtoToMovie()
                }
            }.awaitAll()
        }
    }
    ...
    override suspend fun getQrCodeImage(movie: Movie): String? {
        val response = qrCodeApiService.getQrCodeImage(
